...
function GlobalTab({
  ...
  const totalStars = Object.values(puzzleProgress).reduce(
    (s, p) => s + (p.stars ?? 0),
    0,
  );
  const totalPoints = Object.values(puzzleProgress).reduce(
    (s, p) => s + (p.points ?? 0),
    0,
  );
